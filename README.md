...
- 调度（`route`）：
  输入用户问题 + 12 个 Agent manifest，输出 1~4 步最短计划。
- 回收（`execute`）：
  按计划执行，每步结果写入 `previous_results`，下一步可直接读取上一结果继续加工。
- 统合（`summarize`）：
  只输出最终答案给用户，不暴露 `worker/reviewer` 中间话术。

这就是“中央大脑 + 独立单元 + 单一出口”的最简闭环。

## 路基补强（但不变复杂）

- 计划校验：非法 Agent、空任务会被自动清洗或回退。
- 步骤上限：最多 4 步，避免无限扩张。
- 步骤超时：单步超时自动失败，不拖垮全局（`OFFICETOOL_AGENT_STEP_TIMEOUT_SEC`，默认 25 秒）。
- 上下文收敛：仅保留最近结果链路，防止上下文膨胀。
- 离线稳态：云端不可用时也会给用户可执行答复，不抛底层连接错误。
