...
- `插件独立运行`：`POST /api/agent-plugins/run` 可按 `plugin_id` 单独执行插件；对支持 Swarm 的插件，可通过 `context.swarm.enabled=true` 触发父子分支编排（`max_depth / max_children / join_policy / failure_policy`）。
